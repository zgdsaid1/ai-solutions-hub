...
export default function MarketingModule() {
  ...
  const generateCampaign = async () => {
    ...
    try {
      ...
      if (!session) {
        alert('Please log in to generate campaigns');
        setLoading(false);
        return;
      }

      // Create marketing campaign prompt for AI Router
      const prompt = `Create a comprehensive marketing campaign plan for:

Business Type: ${formData.businessType}
Target Audience: ${formData.targetAudience}
Campaign Goals: ${formData.campaignGoals}
Budget: ${formData.budget}
Timeline: ${formData.timeline} days
Channels: ${formData.channels.join(', ')}
Brand Info: ${formData.brandInfo}
...
4. Channel-specific strategies and tactics
5. Content calendar suggestions
6. Budget allocation recommendations
7. Success metrics and KPIs
8. Timeline and milestones

Make it practical and actionable with specific recommendations.`;

      // Call AI Router with proper task type
      const { data, error } = await supabase.functions.invoke('ai-router', {
        body: {
          prompt,
          taskType: 'marketing_campaign_generation',
          preferredProvider: 'gemini' // Use Gemini for complex marketing strategies
        },
        headers: {
      ...
      const generatedCampaign = parseAIResponseToCampaign(aiResponse, formData);
      
      setCampaign({ ...generatedCampaign, isFallback: false, aiGenerated: true });
      setStep(4);
    } catch (error: any) {
      console.error('Campaign generation error:', error);
      // Generate a fallback campaign when AI fails
  ...
  const parseAIResponseToCampaign = (aiResponse: string, formData: CampaignFormData) => {
    // Create structured campaign from AI response
    return {
      campaignName: `${formData.businessType} ${formData.campaignGoals} Campaign`,
      overview: aiResponse.substring(0, 500) + '...', // First part as overview
      fullStrategy: aiResponse,
      ...
      keyMessages: [
        'Crafted by AI for maximum impact',
        'Tailored to your specific audience',
        'Data-driven strategy approach'
      ],
      contentSuggestions: [
        'Blog posts highlighting value propositions',
    ...
    return {
      ...
      contentStrategy: {
        socialMediaPosts: [
          {
            platform: 'LinkedIn',
            caption: `🚀 ${campaignGoals} for ${businessType}! Helping ${targetAudience} achieve measurable results. Our approach focuses on ${channels.slice(0, 2).join(' and ')}. ${brandInfo ? `Committed to ${brandInfo}.` : 'Dedicated to your success.'}`,
            hashtags: [`#${businessType.replace(/\s+/g, '')}`, '#Growth', '#Marketing']
          },
          {
            platform: 'Twitter', 
            caption: `💡 Proven ${campaignGoals.toLowerCase()} strategies for ${businessType}. ${targetAudience} trust our methods for real results.`,
            hashtags: [`#${campaignGoals.replace(/\s+/g, '')}`, '#BusinessGrowth']
          },
          {
            platform: 'Instagram',
            caption: `✨ ${businessType} success story: How we help ${targetAudience} achieve ${campaignGoals.toLowerCase()} faster.`,
...
Generated by AI Solutions Hub`;
  ...
  const generateReadableContent = (campaign: any) => {
    ...
    content += 'MARKETING CAMPAIGN: ' + (campaign.campaignName || 'Marketing Campaign') + '\n';
    content += 'Generated by AI Solutions Hub - ' + new Date().toLocaleDateString() + '\n\n';
    
    content += 'CAMPAIGN OVERVIEW\n';
    content += '================\n';
    content += (campaign.overview || 'No overview available') + '\n\n';
    
    content += 'CAMPAIGN DETAILS\n';
    content += '===============\n';
    content += 'Business Type: ' + (formData.businessType || 'Not specified') + '\n';
    content += 'Target Audience: ' + (formData.targetAudience || 'Not specified') + '\n';
    content += 'Campaign Goals: ' + (formData.campaignGoals || 'Not specified') + '\n';
    content += 'Budget: ' + (formData.budget || 'Not specified') + '\n';
    content += 'Timeline: ' + (formData.timeline || '30') + ' days\n';
    content += 'Marketing Channels: ' + (formData.channels && formData.channels.length > 0 ? 
      formData.channels.map((channel: string) => channel.replace('_', ' ')).join(', ') : 'No channels specified') + '\n\n';
    ...
    if (campaign.contentStrategy) {
      ...
      if (campaign.contentStrategy.socialMediaPosts) {
        ...
        campaign.contentStrategy.socialMediaPosts.forEach((post: any, idx: number) => {
          content += '  ' + post.platform + ':\n';
          content += '  Caption: ' + post.caption + '\n';
          content += '  Hashtags: ' + (post.hashtags ? post.hashtags.join(', ') : 'None') + '\n';
        });
        content += '\n';
      }
      ...
      if (campaign.contentStrategy.emailSequence) {
        ...
        campaign.contentStrategy.emailSequence.forEach((email: any, idx: number) => {
          content += '  Email ' + (idx + 1) + ': ' + email.subject + '\n';
      ...
      if (campaign.contentStrategy.adCopy) {
        ...
        if (campaign.contentStrategy.adCopy.headlines) {
          content += campaign.contentStrategy.adCopy.headlines.map((headline: string) => '    • ' + headline).join('\n');
        }
        content += '\n  Descriptions:\n';
        if (campaign.contentStrategy.adCopy.descriptions) {
          content += campaign.contentStrategy.adCopy.descriptions.map((desc: string) => '    • ' + desc).join('\n');
        }
        content += '\n';
      }
    }
    ...
    if (campaign.actionPlan) {
      ...
      content += '=================\n';
      Object.entries(campaign.actionPlan).forEach(([week, tasks]: [string, any]) => {
        const weekName = week.replace(/([A-Z])/g, ' $1').trim();
        content += weekName + ':\n';
        if (Array.isArray(tasks)) {
          content += tasks.map((task: string) => '  • ' + task).join('\n');
        } else {
          content += '  • ' + tasks;
        }
        content += '\n\n';
    ...
    if (campaign.metrics) {
      ...
      if (campaign.metrics.kpis) {
        content += campaign.metrics.kpis.map((kpi: string) => '• ' + kpi).join('\n');
      }
      content += '\n\nTRACKING TOOLS\n=============\n';
      if (campaign.metrics.trackingTools) {
        content += campaign.metrics.trackingTools.map((tool: string) => '• ' + tool).join('\n');
      }
      content += '\n\n';
    }
    ...
    if (campaign.channelStrategy) {
      ...
      content += '===============\n';
      campaign.channelStrategy.forEach((channel: any, idx: number) => {
        content += channel.channel + ':\n';
        content += '  Strategy: ' + channel.strategy + '\n';
        content += '  Budget: ' + channel.budget + '\n';
        content += '  KPIs: ' + (channel.kpis ? channel.kpis.join(', ') : 'Not specified') + '\n\n';
      });
    }
    ...
    content += 'For more AI-powered marketing tools, visit our platform\n';
    content += 'Visit: https://b5nss1sw05jo.space.minimax.io\n';
                  ...
                  {campaign.isFallback && (
                    <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      <p className="text-sm text-blue-700">
                        📝 <strong>Note:</strong> This is a quick preview of your campaign. A more detailed AI-generated version will be available soon.
                      </p>
                    </div>
                  )}
                </div>
                        ...
                        {campaign.contentStrategy.socialMediaPosts.map((post: any, idx: number) => (
                          <div key={idx} className="bg-gradient-to-br from-blue-50 to-purple-50 p-4 rounded-lg border border-blue-200">
                            <div className="flex items-center gap-2 mb-2">
                              <span className="text-2xl">{post.platform === 'LinkedIn' ? '💼' : post.platform === 'Twitter' ? '🐦' : '📱'}</span>
